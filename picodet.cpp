...
PicoDet::PicoDet(const char *param, const char *bin, int input_width,
                 int input_height, float score_threshold_ = 0.5,
...
int PicoDet::detect(cv::Mat image, std::vector<BoxInfo> &result_list,
    ...
    if (has_postprocess) {
        ...
        for (const auto &head_info: this->non_postprocess_heads_info) {
            ...
            stride = head_info.stride;
#endif
            std::cout << stride << std::endl;
            std::cout << head_info[0] << std::endl;
            std::cout << head_info[1] << std::endl;
            this->decode_infer(cls_pred, dis_pred, stride,
